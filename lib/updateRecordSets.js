    ...
    return Promise.map(resourceSpec.Instances, function (instanceSpec) {
        ...
        return findInstances(resourceEC2(AWS, instanceSpec), instanceSpec.Filters)
            .then(function (instances) {
                return instanceIps(instances, instanceSpec.PrivateIP);
            });
    }).then(function (ipLists) {
        return changeTemplate(recordSetTemplate(resourceSpec.ResourceRecordSet, _.flatten(ipLists)));
    });
};

var updateResources = module.exports = Promise.method(function (AWS, resource) {
    ...
    return Promise.map(_.pairs(resources), function (pair) {
        ...
        var nonEmptyChange = changes
            && changes.ResourceRecordSet
            && changes.ResourceRecordSet.ResourceRecords
            && changes.ResourceRecordSet.ResourceRecords.length;
        if (!nonEmptyChange) {
            console.error('Cowardly refusing to make update: ', JSON.stringify(changes, null, 4));
            return false;
        } else {
            return true;
        }
